...
async def upload_pdf(file: UploadFile = File(...)):
    ...
    await run_in_threadpool(get_vector_store, file_path)

    return {"filename": safe_filename, "detail": "File uploaded and processed successfully."}
...
async def ask_system(query: str = Form(...), filename: str = Form(None)):
    ...
    safe_filename = _sanitize_pdf_filename(filename) if pdf_uploaded else None
    
    # 1. Get routing decision from Controller
    controller_decision = await run_in_threadpool(decide_route, query, pdf_uploaded)
    ...
    final_answer = await run_in_threadpool(synthesize_answer, query, context_data)
